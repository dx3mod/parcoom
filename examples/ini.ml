...
let wss : string Parcoom.parser = parse_while is_space
...
let section : section_t Parcoom.parser =
  section_name <*> many pair |> map (fun (name, pairs) -> { name; pairs })

let ini : section_t list Parcoom.parser = many section

let () =
  match Sys.argv |> Array.to_list with
  | _ :: file_path :: _ -> (
      let result = file_path |> read_whole_file |> Parcoom.run ini in
      match result with
      | Ok sections -> sections |> show_sections |> print_endline
      | Error error ->
          Printf.printf "Error during parsing at position %d: %s" error.pos
            error.desc)
